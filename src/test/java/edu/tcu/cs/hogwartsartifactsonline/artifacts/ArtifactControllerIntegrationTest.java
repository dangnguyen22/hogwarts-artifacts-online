...
public class ArtifactControllerIntegrationTest {
    ...
    void setUp() throws Exception {
        ResultActions resultActions = this.mockMvc.
                perform(post(this.baseUrl + "/users/login").with(httpBasic("admin", "admin")));

        MvcResult mvcResult = resultActions.andDo(print()).andReturn();
        String contentasString = mvcResult.getResponse().getContentAsString();
        JSONObject jsonObject = new JSONObject(contentasString);
        this.token = "Bearer " + jsonObject.getJSONObject("data").getString("token");//Don't forget to add "Bearer " before the token
